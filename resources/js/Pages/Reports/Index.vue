<template>
    <MainLayout
        page-title="Rapports"
        page-description="Tableau de bord et statistiques"
    >
        <!-- Period Filter -->
        <Card class="mb-6">
        ...
        <div v-if="activeTab === 'sales'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard
                    label="Panier moyen"
                    :value="salesData.average_order"
                    format="currency"
                    :currency="reportCurrency"
                />
                <StatCard
                    label="Commandes annulées"
                    :value="salesData.canceled_orders"
                    format="number"
                    variant="danger"
                />
            </div>
        ...
        <div v-if="activeTab === 'products'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard
                    ...
                    :value="productsData.total_items_sold"
                    format="number"
                />
                <StatCard
                    label="Produits différents"
                    :value="productsData.unique_products"
                    format="number"
                />
                <StatCard
                    label="Meilleur vendeur"
                    :value="productsData.top_product?.name || '-'"
                    format="text"
                    variant="success"
                />
                <StatCard
                    label="Revenu produits"
                    :value="productsData.total_revenue"
                    format="currency"
                    :currency="reportCurrency"
                />
            </div>
        ...
        <div v-if="activeTab === 'servers'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard
                    ...
                    :value="serversData.active_servers"
                    format="number"
                />
                <StatCard
                    label="Total commandes"
                    :value="serversData.total_orders"
                    format="number"
                />
                <StatCard
                    label="Meilleur serveur"
                    :value="serversData.top_server?.name || '-'"
                    format="text"
                    variant="success"
                />
                <StatCard
                    label="Chiffre moyen/serveur"
                    :value="serversData.average_revenue"
                    format="currency"
                    :currency="reportCurrency"
                />
            </div>
        ...
        <div v-if="activeTab === 'sessions'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard
                    ...
                    :value="sessionsData.closed_sessions"
                    format="number"
                />
                <StatCard
                    label="Total encaissé"
                    :value="sessionsData.total_collected"
                    format="currency"
                    :currency="reportCurrency"
                    variant="success"
                />
                <StatCard
                    label="Écart total"
                    :value="sessionsData.total_difference"
                    format="currency"
                    :currency="reportCurrency"
                    :variant="
                        sessionsData.total_difference >= 0
                            ? 'success'
                            : 'danger'
                    "
                />
        ...
        <div v-if="activeTab === 'stocks'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard
                    ...
                    :value="stocksData.total_ingredients"
                    format="number"
                />
                <StatCard
                    label="En alerte"
                    :value="stocksData.low_stock_count"
                    format="number"
                    variant="danger"
                />
                <StatCard
                    label="Valeur du stock"
                    :value="stocksData.total_value"
                    format="currency"
                    :currency="reportCurrency"
                />
                <StatCard
                    label="Mouvements"
                    :value="stocksData.movements_count"
                    format="number"
                />
            </div>
            ...
            <Card title="Alertes de stock">
                ...
                <EmptyState
                    v-else
                    title="Aucune alerte"
                    description="Tous les stocks sont à un niveau suffisant"
                />
            </Card>
            ...
            <Card title="Mouvements récents">
                <Table
                    ...
                    :data="stocksData.recent_movements || []"
                >
                    <template #cell-type="{ value }">
                        <Badge
                            :variant="value === 'in' ? 'success' : 'warning'"
                        >
        ...
        <div v-if="activeTab === 'activity'" class="space-y-6">
            <Card title="Journal d'activité">
                <div class="space-y-3">
                    ...
                    >
                        <div
                            :class="getActivityIconClass(log.action)"
                ...
                <EmptyState
                    v-if="!activityData.logs?.length"
                    title="Aucune activité"
                    description="Aucune activité enregistrée pour cette période"
                />
            </Card>
        </div>
    </MainLayout>
