        ...
        <div class="p-4 border-b" :class="headerBg">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div
                        class="flex items-center justify-center w-12 h-12 bg-white rounded-lg shadow"
                    >
                    ...
                    <div>
                        ...
                        >
                            ...
                            <span :class="timeColorClass" class="font-medium">
                                {{ formattedTime }}
                            </span>
                        </div>
                    </div>
        ...
        <div class="divide-y">
            ...
            >
                <div class="flex items-center gap-4">
                    <!-- Statut visuel -->
                    <div
                        :class="getItemStatusClass(item.kitchen_status)"
                ...
                <!-- Boutons d'action par item -->
                <div class="flex items-center gap-2">
                    <button
                        v-if="item.kitchen_status === 'waiting'"
                        @click="
                            $emit('update-status', {
                                itemId: item.id,
                                status: 'preparing',
                        ...
                        "
                        class="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition text-sm"
                    >
                        ▶️ Démarrer
                    </button>
                    <button
                        v-else-if="item.kitchen_status === 'preparing'"
                        @click="
                            $emit('update-status', {
                                itemId: item.id,
                                status: 'ready',
                    ...
                    <span
                        v-else-if="item.kitchen_status === 'ready'"
                        class="px-3 py-1 bg-green-500 text-white rounded-lg text-sm"
                    >
                        ✓ Prêt
                    </span>
                    <span
                        v-else-if="item.kitchen_status === 'served'"
...
const canMarkAllReady = computed(() => {
    ...
    return items.some(
        (i) =>
            i.kitchen_status === "waiting" || i.kitchen_status === "preparing"
    );
});
