<template>
  <div :class="alertClasses" class="rounded-lg p-4" role="alert">
    <div class="flex">
      <!-- Icon -->
      <div v-if="showIcon" class="flex-shrink-0">
        <component :is="iconComponent" class="h-5 w-5" />
      </div>

      <!-- Content -->
      <div :class="showIcon ? 'ml-3' : ''" class="flex-1">
        <h3 v-if="title" class="text-sm font-medium">{{ title }}</h3>
        <div :class="title ? 'mt-1' : ''" class="text-sm">
          <slot />
        </div>
      </div>

      <!-- Close Button -->
      <div v-if="dismissible" class="ml-auto pl-3">
        <button
          type="button"
          :class="closeButtonClasses"
          class="-mx-1.5 -my-1.5 rounded-lg p-1.5 inline-flex h-8 w-8 focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors"
          @click="$emit('dismiss')"
        >
          <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fill-rule="evenodd"
              d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
              clip-rule="evenodd"
            />
          </svg>
        </button>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed, h } from 'vue';

const props = defineProps({
  variant: {
    type: String,
    default: 'info',
    validator: (value) => ['info', 'success', 'warning', 'danger'].includes(value),
  },
  title: {
    type: String,
    default: '',
  },
  showIcon: {
    type: Boolean,
    default: true,
  },
  dismissible: {
    type: Boolean,
    default: false,
  },
});

defineEmits(['dismiss']);

const variantClasses = {
  info: 'bg-blue-50 text-blue-800',
  success: 'bg-green-50 text-green-800',
  warning: 'bg-yellow-50 text-yellow-800',
  danger: 'bg-red-50 text-red-800',
};

const closeButtonVariants = {
  info: 'bg-blue-50 text-blue-500 hover:bg-blue-100 focus:ring-blue-600',
  success: 'bg-green-50 text-green-500 hover:bg-green-100 focus:ring-green-600',
  warning: 'bg-yellow-50 text-yellow-500 hover:bg-yellow-100 focus:ring-yellow-600',
  danger: 'bg-red-50 text-red-500 hover:bg-red-100 focus:ring-red-600',
};

const alertClasses = computed(() => variantClasses[props.variant]);
const closeButtonClasses = computed(() => closeButtonVariants[props.variant]);

// Icons for each variant
const icons = {
  info: {
    render: () =>
      h('svg', { fill: 'currentColor', viewBox: '0 0 20 20' }, [
        h('path', {
          'fill-rule': 'evenodd',
          d: 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z',
          'clip-rule': 'evenodd',
        }),
      ]),
  },
  success: {
    render: () =>
      h('svg', { fill: 'currentColor', viewBox: '0 0 20 20' }, [
        h('path', {
          'fill-rule': 'evenodd',
          d: 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z',
          'clip-rule': 'evenodd',
        }),
      ]),
  },
  warning: {
    render: () =>
      h('svg', { fill: 'currentColor', viewBox: '0 0 20 20' }, [
        h('path', {
          'fill-rule': 'evenodd',
          d: 'M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z',
          'clip-rule': 'evenodd',
        }),
      ]),
  },
  danger: {
    render: () =>
      h('svg', { fill: 'currentColor', viewBox: '0 0 20 20' }, [
        h('path', {
          'fill-rule': 'evenodd',
          d: 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z',
          'clip-rule': 'evenodd',
        }),
      ]),
  },
};

const iconComponent = computed(() => icons[props.variant]);
</script>
