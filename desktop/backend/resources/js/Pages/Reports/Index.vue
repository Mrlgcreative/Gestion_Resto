    ...
    <MainLayout page-title="Rapports" page-description="Tableau de bord et statistiques">
        ...
        <div class="mb-6 border-b border-gray-200">
            <nav class="-mb-px flex gap-4">
                <button v-for="tab in tabs" :key="tab.id"
                    class="py-2.5 px-4 border-b-2 font-medium text-sm transition-colors"
        ...
        <div v-if="activeTab === 'sales'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Ventes par jour</h3>
                <div class="h-72">
                    <SalesChart :data="salesData.daily_sales || []" :show-orders="true" />
                </div>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Détails des ventes</h3>
                <Table :columns="salesColumns" :data="salesData.daily_sales || []">
                    <template #cell-revenue="{ value }">{{ formatPrice(value) }}</template>
                    <template #cell-average="{ value }">{{ formatPrice(value) }}</template>
                </Table>
            </div>
        </div>
        ...
        <div v-if="activeTab === 'products'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Palmarès des produits</h3>
        ...
        <div v-if="activeTab === 'servers'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Performance des serveurs</h3>
        ...
        <div v-if="activeTab === 'sessions'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Historique des sessions</h3>
                <Table :columns="sessionColumns" :data="sessionsData.sessions || []">
                    <template #cell-user="{ row }">{{ row.user?.name }}</template>
                    <template #cell-opening_amount="{ value }">{{ formatPrice(value) }}</template>
                    <template #cell-closing_amount="{ value }">{{ value ? formatPrice(value) : "-" }}</template>
                    <template #cell-difference="{ row }">
                        <span v-if="row.closing_amount" :class="row.closing_amount - row.expected_amount >= 0 ? 'text-emerald-600' : 'text-red-600'">
        ...
        <div v-if="activeTab === 'stocks'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Alertes de stock</h3>
                <div v-if="stocksData.low_stock?.length > 0" class="space-y-2">
                    <div v-for="item in stocksData.low_stock" :key="item.id" class="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                        <div>
                            <p class="font-medium text-red-900">{{ item.name }}</p>
                            <p class="text-sm text-red-600">Quantité: {{ item.quantity }} {{ item.unit }}</p>
                        </div>
                        <Badge variant="danger">Seuil: {{ item.alert_level }} {{ item.unit }}</Badge>
                    </div>
                </div>
                <EmptyState v-else title="Aucune alerte" description="Tous les stocks sont à un niveau suffisant" />
            </div>
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Mouvements récents</h3>
                <Table :columns="movementColumns" :data="stocksData.recent_movements || []">
                    <template #cell-type="{ value }">
                        <Badge :variant="value === 'in' ? 'success' : 'warning'">{{ value === "in" ? "Entrée" : "Sortie" }}</Badge>
                    </template>
                    <template #cell-ingredient="{ row }">{{ row.ingredient?.name }}</template>
        ...
        <div v-if="activeTab === 'activity'" class="space-y-6">
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Journal d'activité</h3>
                <div class="space-y-3">
                    <div v-for="log in activityData.logs || []" :key="log.id" class="flex items-start gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <Badge :variant="getActivityVariant(log.action)" size="sm">{{ getActivityCategory(log.action) }}</Badge>
                            </div>
...
const getActivityIconClass = (action) => {
    ...
    if (action.includes("created") || action.includes("_created")) return "bg-emerald-100 text-emerald-600";
    if (action.includes("deleted") || action.includes("canceled") || action.includes("_deleted")) return "bg-red-100 text-red-600";
    if (action.includes("updated") || action.includes("_updated")) return "bg-amber-100 text-amber-600";
    if (action.includes("paid") || action.includes("session_closed")) return "bg-emerald-100 text-emerald-600";
    if (action.includes("stock")) return "bg-purple-100 text-purple-600";
    return "bg-gray-100 text-gray-600";
};

const getActivityIcon = (action) => { return "svg"; };
const getActivityVariant = (action) => {
    if (action.includes("created") || action.includes("_created") || action.includes("paid")) return "success";
    if (action.includes("deleted") || action.includes("canceled") || action.includes("_deleted")) return "danger";
    if (action.includes("updated") || action.includes("_updated")) return "warning";
    if (action.includes("login") || action.includes("logout")) return "info";
    return "default";
};
const getActivityCategory = (action) => {
    if (action.includes("login") || action.includes("logout")) return "Authentification";
    if (action.includes("order")) return "Commande";
    if (action.includes("session")) return "Caisse";
    if (action.includes("stock")) return "Stock";
    if (action.includes("product")) return "Produit";
    if (action.includes("user")) return "Utilisateur";
    if (action.includes("settings") || action.includes("options")) return "Paramètres";
    if (action.includes("currency") || action.includes("exchange")) return "Devise";
    if (action.includes("created")) return "Création";
    if (action.includes("updated")) return "Modification";
    if (action.includes("deleted")) return "Suppression";
    return "Système";
};

const applyFilters = () => { router.get("/reports", filters, { preserveState: true }); };
