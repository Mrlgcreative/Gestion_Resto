...
class ReportController extends Controller implements HasMiddleware
{
    ...
    {
        $query = Order::forService(auth()->user()->service_id)->where('status', 'paid')
        ...
        $canceledQuery = Order::forService(auth()->user()->service_id)->where('status', 'canceled')
            ->whereBetween('created_at', [$startDate, $endDate]);
        if ($userId) {
        ...
        $dailySalesQuery = Order::forService(auth()->user()->service_id)->where('status', 'paid')
            ->whereBetween('created_at', [$startDate, $endDate]);
        if ($userId) {
    ...
    private function getServersData($startDate, $endDate, $reportCurrency = 'USD')
    {
        ...
        $orders = Order::forService(auth()->user()->service_id)->where('status', 'paid')
            ->whereBetween('created_at', [$startDate, $endDate])
